...
export class ListsService {
  ...
  async findAll(user: User, skip: number = 0, take: number = 20, search?: string) {
    ...
    if (sort) {
      ...
      allProblems.sort((a, b) => {
        ...
        if (sort === 'title') {
          ...
          valA = diffMap[a.problem.difficulty] ?? 0;
          valB = diffMap[b.problem.difficulty] ?? 0;
        } else if (sort === 'status') {
          const statusMapVal: Record<string, number> = { Todo: 1, Attempted: 2, Solved: 3 };
          valA = statusMapVal[a.problem.status] ?? 0;
          valB = statusMapVal[b.problem.status] ?? 0;
        }
  ...
  async addProblems(id: string, dto: ManageListProblemsDto, user: User) {
  ...
  async removeProblems(id: string, dto: ManageListProblemsDto, user: User) {
    if (!dto.problemIds || dto.problemIds.length === 0) {
      return { count: 0 };
    }
