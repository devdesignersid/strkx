    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          />
          {/* Centering wrapper - flexbox approach for proper max-height behavior */}
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="w-full max-w-md max-h-[80vh] bg-[#1e1e1e] border border-white/10 rounded-xl shadow-2xl flex flex-col overflow-hidden"
            >
              <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-white/5 bg-[#252526]">
              ...
              <div className="shrink-0 p-4 border-t border-white/5 bg-[#252526] flex justify-end gap-3">
                <button
                  onClick={onClose}
