  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (sessionId && session && session.status !== 'COMPLETED' && session.status !== 'ABANDONED' && !isFinished.current) {
        ...
        // Attempt to abandon immediately using fetch with keepalive
        const url = `${API_URL}/interview-sessions/${sessionId}/abandon`;
        fetch(url, {
          method: 'POST',
          credentials: 'include',
  ...
  const handleSubmit = useCallback(async (auto = false) => {
    ...
    try {
      ...
      try {
        ...
        const execResponse = await execRes.json();
        // Backend wraps response in a "data" object via TransformInterceptor
      ...
      if (mounted.current) {
        ...
        // Note: We DO NOT reset submittingQuestionId.current here if successful,
        // because we don't want to allow re-submission of the same question ID ever.
        // It will be reset when we move to the next question (which has a different ID).
        // However, if we failed, we might want to reset it?
        // Actually, if we failed with 400 "already completed", we definitely don't want to reset it.
        // If we failed with 500, maybe?
        // For safety, let's only reset it if we are NOT navigating away/forward.
        // But since we navigate away on success, it doesn't matter much.
        // If we stay on the page (error), we might want to retry?
        // Let's reset it ONLY if we are NOT finished/moving next.
        // But checking that state is hard here.
        // Let's just reset it if we caught an error that wasn't "already completed".
      }
    }
  }, [session, sessionId, currentQuestionIndex, code, isSubmitting, navigate]);
